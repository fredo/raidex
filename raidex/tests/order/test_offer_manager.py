...
def test_add_offer(offer_manager, internal_offer):
    ...
    assert not offer_manager.has_offer(internal_offer.order_id)
    assert offer_manager.get_offer(internal_offer.order_id) is None

    offer_manager.add_offer(internal_offer)

    assert offer_manager.has_offer(internal_offer.order_id)
    assert offer_manager.get_offer(internal_offer.order_id) == internal_offer


def test_add_take_offer(offer_manager, basic_offer):

    assert not offer_manager.has_offer(basic_offer.order_id)
    assert offer_manager.get_offer(basic_offer.order_id) is None

    take_offer = offer_manager.create_take_offer(basic_offer)

    assert take_offer == basic_offer
    assert offer_manager.has_offer(basic_offer.order_id)
    assert offer_manager.get_offer(basic_offer.order_id) == basic_offer
...
def test_add_make_offer(offer_manager, limit_order, amount_left):
    ...
    assert offer_manager.has_offer(offer.order_id)
    assert offer_manager.get_offer(offer.order_id) == offer
